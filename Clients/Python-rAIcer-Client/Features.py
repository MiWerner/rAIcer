...
def is_free_horizontal(mask, posX, posY, direction, length):
    for offset in range(length):
        if mask[posX + offset * direction, posY]:
            return True
    return False


def is_free_vertical(mask, posX, posY, direction, length):
    for offset in range(length):
        if mask[posX, posY + offset * direction]:
            return True
    return False
...
def _calc_distance_up(ball_pos, track_mask):
    ...
    while is_free_horizontal(track_mask, ball_pos[0], current_y, -1, 10):
        current_y -= 1

    return ball_pos[1] - current_y  # ball_pos[1] is larger or equal to current_x
...
def _calc_distance_down(ball_pos, track_mask):
    ...
    return current_y - ball_pos[1] # current_y is larger or equal to ball_pos[1]
...
def _calc_distance_left(ball_pos, track_mask):
    ...
    while is_free_horizontal(track_mask, current_x, ball_pos[1], -1, 8):
        current_x -= 1

    return ball_pos[0] - current_x  # ball_pos[0] is larger or equal to current_x
...
def _calc_distance_right(ball_pos, track_mask):
    ...
    return current_x - ball_pos[0]  # current_x is larger or equal to ball_pos[0]
